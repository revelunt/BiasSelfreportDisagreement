...
## overall candidate preference change over time
## within the panel, 82% of respondents never change the favored candidate
cleaned.data[canpref.W1 == 0 & canpref.W2 == 0 & canpref.W3 == 0, .N]
cleaned.data[canpref.W1 == 1 & canpref.W2 == 1 & canpref.W3 == 1, .N]

## from published opinion surveys, we also see pretty consistant trends over time
## see: https://bit.ly/2MP3Ue7

survey_results <- data.frame(
  order = 1:14,
  date = c("11.27", "12.02", "12.05", "12.06", "12.07", "12.08", "12.12 (1)", "12.12 (2)",
           "12.12 (3)", "12.12 (4)", "12.13 (1)", "12.13 (2)", "12.13 (3)", "12.13 (4)"),
  park = c(45.4, 44.9, 50.6, 43.5, 48.9, 49.5, 45.3, 46.3, 42.8, 44.9, 48.5, 48, 45.4, 44.9),
  moon = c(43.8, 40.9, 43.3, 40.2, 42.8, 42.5, 41.4, 40.3, 41.9, 41.4, 46.9, 47.5, 43.4, 45.3)
) %>% setDT

survey_results <-
  data.table::melt(survey_results, id.vars = c("order", "date"),
                   variable.name = "Candidate",
                   value.name = "favorability")

ggplot(survey_results, aes(x = date, y = favorability,
                           color = Candidate)) +
  geom_point() + geom_line(aes(group = Candidate)) +
  scale_y_continuous(labels = function(x) paste0(x, "%"),
                     limits = c(0, 100)) +
  ylab("Candidate favorability ratings") +
  xlab("Date of survey") + theme_bw()
...
## reading count
figure_r1 <- data.frame(
  Type = c("Reading", "Reading", "Reading", "Reading",
           "Writing", "Writing", "Writing", "Writing"),
  Estimates = c(cor.test(net[, sum(count), by = reader.id][, V1],
                      cleaned.data[, pol.know])$estimate,
               cor.test(net[, sum(count), by = reader.id][, V1],
                        cleaned.data[, pol.interest.W1])$estimate,
               cor.test(net[, sum(count), by = reader.id][, V1],
                        cleaned.data[, pol.interest.W2])$estimate,
               cor.test(net[, sum(count), by = reader.id][, V1],
                        cleaned.data[, pol.interest.W3])$estimate,
               cor.test(dat[, 쓰기_sum], cleaned.data[, pol.know])$estimate,
               cor.test(dat[, 쓰기_sum], cleaned.data[, pol.interest.W1])$estimate,
               cor.test(dat[, 쓰기_sum], cleaned.data[, pol.interest.W2])$estimate,
               cor.test(dat[, 쓰기_sum], cleaned.data[, pol.interest.W3])$estimate),
  llci = c(cor.test(net[, sum(count), by = reader.id][, V1],
                    cleaned.data[, pol.know])$conf[1],
          cor.test(net[, sum(count), by = reader.id][, V1],
                   cleaned.data[, pol.interest.W1])$conf[1],
          cor.test(net[, sum(count), by = reader.id][, V1],
                   cleaned.data[, pol.interest.W2])$conf[1],
          cor.test(net[, sum(count), by = reader.id][, V1],
                    cleaned.data[, pol.interest.W3])$conf[1],
          cor.test(dat[, 쓰기_sum], cleaned.data[, pol.know])$conf[1],
          cor.test(dat[, 쓰기_sum], cleaned.data[, pol.interest.W1])$conf[1],
          cor.test(dat[, 쓰기_sum], cleaned.data[, pol.interest.W2])$conf[1],
          cor.test(dat[, 쓰기_sum], cleaned.data[, pol.interest.W3])$conf[1]),
  ulci = c(cor.test(net[, sum(count), by = reader.id][, V1],
                    cleaned.data[, pol.know])$conf[2],
           cor.test(net[, sum(count), by = reader.id][, V1],
                    cleaned.data[, pol.interest.W1])$conf[2],
           cor.test(net[, sum(count), by = reader.id][, V1],
                    cleaned.data[, pol.interest.W2])$conf[2],
           cor.test(net[, sum(count), by = reader.id][, V1],
                    cleaned.data[, pol.interest.W3])$conf[2],
           cor.test(dat[, 쓰기_sum], cleaned.data[, pol.know])$conf[2],
           cor.test(dat[, 쓰기_sum], cleaned.data[, pol.interest.W1])$conf[2],
           cor.test(dat[, 쓰기_sum], cleaned.data[, pol.interest.W2])$conf[2],
           cor.test(dat[, 쓰기_sum], cleaned.data[, pol.interest.W3])$conf[2]),
  cor.with = c("know_W1", "interest_W1", "interest_W2", "interest_W3",
               "know_W1", "interest_W1", "interest_W2", "interest_W3"))

ggplot(figure_r1, aes(y = Estimates, x = Type)) +
  geom_point(aes(color = factor(cor.with)),
             position = position_dodge(width = 0.3)) +
  geom_pointrange(aes(color = factor(cor.with)),
                ymin = figure_r1$llci, ymax = figure_r1$ulci,
                position = position_dodge(width = 0.3)) +
  ylim(-0.1, 0.3) + theme_bw() +
  geom_hline(yintercept = 0, linetype = 2, color = "red") +
  guides(color = guide_legend(title = "Correlation with:")) +
  ylab("Zero-order correlations\n(95% CIs in range)") + xlab("")
...
## ----------------------------------------------------- ##
## How sensitive our results to omitted, third variable? ##
## ----------------------------------------------------- ##

ols.model1 <- lm(dis.accuracy.W2.candpref ~ ## predicting overestimation
                   ## social desirability
                   discussion.norm.W2 +
                   #need.for.approval.W2 + ## cf. interaction is not sig as well
                   ## partisan motivations
...
require(sensemakr)
sensitivity <- sensemakr(model = ols.model1,
                         treatment = "perceived.opinion.climate.W2")
                         #benchmark_covariates = "discussion.norm.W2",
                         #kd = 1:3)
plot(sensitivity)


## --------------------------------------------- ##
## Whether perception and actual behaviors were  ##
## under the same interference of three factors? ##
## --------------------------------------------- ##
cleaned.data[, cor.test(dangerous.disc.W1.candpref, dangerous.disc.prcptn.W2.candpref)]

model1.selfreport <- lm(dangerous.disc.W1.candpref ~ ## predicting overestimation
                          ## social desirability
                          discussion.norm.W2 +
                          #need.for.approval.W2 + ## cf. interaction is not sig as well
                          ## partisan motivations
...
model1.behavior <- lm(dangerous.disc.prcptn.W2.candpref ~ ## predicting overestimation
                          ## social desirability
                          discussion.norm.W2 +
                          #need.for.approval.W2 + ## cf. interaction is not sig as well
                          ## partisan motivations
...
model2.selfreport <- lm(dangerous.disc.W2.candpref ~ ## predicting overestimation
                          ## social desirability
                          discussion.norm.W3 +
                          #need.for.approval.W2 + ## cf. interaction is not sig as well
                          ## partisan motivations
...
model2.behavior <- lm(dangerous.disc.prcptn.W3.candpref ~ ## predicting overestimation
                        ## social desirability
                        discussion.norm.W3 +
                        #need.for.approval.W2 + ## cf. interaction is not sig as well
                        ## partisan motivations
...
htmlreg(list(model1.selfreport, model1.behavior,
               model2.selfreport, model2.behavior),
        file = "doc.doc", bold = T, single.row = T,
          digits = 3, leading.zero = FALSE,
          custom.model.names = c("Self-R W2", "Behavior W1",
                                 "Self-R W3", "Behavior W2"),
          custom.coef.names = c(
            "(Intercept)", "Discussion norm W2/W3",
            "Prcvd Op Climate W2/W3",
            "Candidate pref W2/W3", "Ideo Strength W2/W3",
...
## whether the diff of coef itself is statistically significant?
diff.coef1 <- function(data, i) {
  redata <- data[i, ]
  refit.1 <- lm(formula(model1.selfreport), data = redata)
  refit.2 <- lm(formula(model1.behavior), data = redata)

  diff.coef <- coef(refit.1) - coef(refit.2)
  names(diff.coef) <- names(coef(refit.1))
  diff.coef
}

diff.coef2 <- function(data, i) {
  redata <- data[i, ]
  refit.1 <- lm(formula(model2.selfreport), data = redata)
  refit.2 <- lm(formula(model2.behavior), data = redata)
...
require(boot)
test1 <- boot(data = cleaned.data, statistic = diff.coef1, R = 10000,
              parallel = "multicore", ncpus = 4)
test2 <- boot(data = cleaned.data, statistic = diff.coef2, R = 10000,
              parallel = "multicore", ncpus = 4)
coef_diff1 <- apply(test1$t, 2, quantile, c(0.5, 0.025, 0.975)) %>% t %>% as.data.frame
coef_diff2 <- apply(test2$t, 2, quantile, c(0.5, 0.025, 0.975)) %>% t %>% as.data.frame

rownames(coef_diff1) <- names(coef(model1.selfreport))
colnames(coef_diff1) <- c("median", "llci95", "ulci95")
coef_diff1$y <- names(coef(model1.selfreport))
coef_diff1 <- coef_diff1[c(2:5, 7:8), ]
coef_diff1$color <- c("red", "red", "grey", "grey", "grey", "grey")
coef_diff1$y <- factor(coef_diff1$y, levels = rev(coef_diff1$y))

rownames(coef_diff2) <- names(coef(model2.selfreport))
colnames(coef_diff2) <- c("median", "llci95", "ulci95")
coef_diff2$y <- names(coef(model2.selfreport))
coef_diff2 <- coef_diff2[c(2:5, 7:8), ]
coef_diff2$color <- c("red", "red", "grey", "grey", "grey", "grey")
coef_diff2$y <- factor(coef_diff2$y, levels = rev(coef_diff2$y))

figure_r2a <-
  ggplot(coef_diff1, aes(y = y, x = median)) +
    geom_point(color = coef_diff1$color) +
    geom_linerange(aes(xmin = llci95, xmax = ulci95),
                   color = coef_diff1$color) +
    geom_vline(xintercept = 0, linetype = 2) +
    theme_bw() + ylab("") + xlab("Differences in coef") +
  ggtitle("W1 behavior vs. W2 perception")

figure_r2b <-
  ggplot(coef_diff2, aes(y = y, x = median)) +
  geom_point(color = coef_diff2$color) +
  geom_linerange(aes(xmin = llci95, xmax = ulci95),
                 color = coef_diff2$color) +
  geom_vline(xintercept = 0, linetype = 2) +
  theme_bw() + ylab("") + xlab("Differences in coef") +
  ggtitle("W2 behavior vs. W3 perception")

require(patchwork)
figure_r2a + figure_r2b + plot_layout(nrow = 2)
